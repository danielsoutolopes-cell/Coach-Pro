...
export interface StrengthCatalogItem {
  id: number;
  name: string;
...
export const ProCoachAPI = {
  ...
  async searchStrengthCatalog(opts: { q: string; limit?: number }) {
    const q = encodeURIComponent(opts.q ?? "");
    const limit = opts.limit ? `&limit=${encodeURIComponent(String(opts.limit))}` : "";
    return request<{ items: StrengthCatalogItem[] }>(`/procoach/me/strength/catalog?q=${q}${limit}`);
  },

  async getStrengthTemplates() {
    return request<{ templates: Record<StrengthTemplateCode, StrengthTemplate> }>(`/procoach/me/strength/templates`);
  },

  async upsertStrengthTemplate(
    code: StrengthTemplateCode,
    payload: { name: string; notes?: string | null; exercises: StrengthTemplateExercise[] }
  ) {
    return request<{ ok: boolean; code: StrengthTemplateCode }>(`/procoach/me/strength/templates/${code}`, {
      method: "PUT",
      body: JSON.stringify(payload),
    });
  },
