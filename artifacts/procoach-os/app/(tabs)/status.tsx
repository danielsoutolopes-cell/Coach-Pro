...
export default function CheckInScreen() {
  ...
  const [strengthCode, setStrengthCode] = useState<StrengthTemplateCode>("A");
  const [strengthTemplates, setStrengthTemplates] = useState<Record<StrengthTemplateCode, StrengthTemplate> | null>(null);
  const [strengthName, setStrengthName] = useState("");
  const [strengthNotes, setStrengthNotes] = useState("");
  const [strengthExercises, setStrengthExercises] = useState<StrengthTemplateExercise[]>([]);

  const [exerciseQuery, setExerciseQuery] = useState("");
  const [exerciseSearching, setExerciseSearching] = useState(false);
  const [exerciseSuggestions, setExerciseSuggestions] = useState<StrengthCatalogItem[]>([]);
  ...
  const refreshStrength = useCallback(async () => {
    ...
    try {
      const r = await ProCoachAPI.getStrengthTemplates();
      setStrengthTemplates(r.templates);
    } catch {
      setStrengthTemplates(null);
    } finally {
  ...
  useEffect(() => {
    const t = strengthTemplates?.[strengthCode];
    if (!t) {
      setStrengthName("");
      setStrengthNotes("");
    ...
    setStrengthName(t.name ?? "");
    setStrengthNotes(t.notes ?? "");
    setStrengthExercises(Array.isArray(t.exercises) ? t.exercises : []);
  }, [strengthTemplates, strengthCode]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ProCoachAPI.searchStrengthCatalog({ q, limit: 12 })
        .then((r) => setExerciseSuggestions(r.items ?? []))
  ...
  const addExerciseFromCatalog = useCallback((item: StrengthCatalogItem) => {
  ...
  const removeStrengthExercise = useCallback((idx: number) => {
    setStrengthExercises((prev) => prev.filter((_, i) => i !== idx));
    Haptics.selectionAsync();
  }, []);

  const updateStrengthExercise = useCallback((idx: number, patch: Partial<StrengthTemplateExercise>) => {
    setStrengthExercises((prev) => prev.map((e, i) => (i === idx ? { ...e, ...patch } : e)));
  }, []);

  const handleSaveStrength = useCallback(async () => {
    setStrengthSaving(true);
    try {
      await ProCoachAPI.upsertStrengthTemplate(strengthCode, {
        name: strengthName,
        notes: strengthNotes,
      ...
      Alert.alert("Erro ao salvar ficha", String(e?.message ?? e ?? "Falha desconhecida"));
    } finally {
      setStrengthSaving(false);
    }
  }, [strengthCode, strengthName, strengthNotes, strengthExercises, refreshStrength]);
  ...
  const handleImportPlan = async () => {
    ...
    try {
      ...
      const treinos = maybePlan?.treinos;
      const treinoHoje = maybePlan?.treino_hoje ?? maybePlan?.treinoHoje;
      ...
      if (items.length === 0) {
        Alert.alert(
          "JSON inválido",
          "Não encontrei 'treinos' (ou 'treino_hoje') nem 'cronograma' no JSON. Cole o JSON completo do plano."
        );
        return;
      }
  ...
  const s = StyleSheet.create({
    ...
    strengthChipRow: { flexDirection: "row", gap: 8, marginBottom: 12 },
    strengthChip: { flex: 1, paddingVertical: 10, borderRadius: 10, borderWidth: 1.5, alignItems: "center" },
    strengthChipText: { fontSize: 10, fontWeight: "900" as const, letterSpacing: 2 },
    strengthExerciseRow: { borderTopWidth: 1, paddingTop: 10, marginTop: 10 },
    ...
    removeBtn: { paddingHorizontal: 12, paddingVertical: 10, borderRadius: 10, borderWidth: 1.5, alignItems: "center", justifyContent: "center" },
    removeBtnText: { fontSize: 10, fontWeight: "900" as const, letterSpacing: 2 },
  });
  ...
  return (
    <View style={s.container}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.card}>
          <View style={s.cardTitleRow}>
            <Text style={s.cardTitle}>BIBLIOTECA DE FORÇA (A/B/C)</Text>
            {(strengthLoading || strengthSaving) && <ActivityIndicator size="small" color={colors.mutedForeground} />}
          </View>
          <View style={s.divider} />
          <View style={s.strengthChipRow}>
            {(["A", "B", "C"] as StrengthTemplateCode[]).map((c) => {
              const active = strengthCode === c;
              return (
                <Pressable
                  key={c}
                  style={({ pressed }) => [
                  ...
                  onPress={() => setStrengthCode(c)}
                >
                  <Text style={[s.strengthChipText, { color: active ? colors.primary : colors.mutedForeground }]}>
                    FICHA {c}
                  </Text>
                </Pressable>
              );
          ...
          <Text style={s.profileLabel}>NOME DA FICHA</Text>
          <TextInput
            style={[s.inputField, { fontSize: 14 }]}
            value={strengthName}
            onChangeText={setStrengthName}
            placeholderTextColor={colors.mutedForeground}
            placeholder="Ex.: Inferiores / Anterior + Core / Posterior"
          />
          <View style={{ height: 10 }} />
          <Text style={s.profileLabel}>OBSERVAÇÕES</Text>
          <TextInput
            style={[s.inputField, { fontSize: 12, minHeight: 60, textAlignVertical: "top" as any }]}
            value={strengthNotes}
            onChangeText={setStrengthNotes}
            placeholderTextColor={colors.mutedForeground}
            placeholder="Ex.: foco em estabilidade, sem falha, técnica perfeita"
            multiline
          />

          <View style={{ height: 12 }} />
          <Text style={s.profileLabel}>ADICIONAR EXERCÍCIO (CATÁLOGO)</Text>
          <TextInput
            style={[s.inputField, { fontSize: 13 }]}
            value={exerciseQuery}
            onChangeText={setExerciseQuery}
            placeholderTextColor={colors.mutedForeground}
            placeholder="Digite pelo menos 2 letras (ex.: agacha, supino, remada)"
            autoCapitalize="none"
            autoCorrect={false}
          />
          {exerciseSearching && (
            <Text style={{ marginTop: 8, fontSize: 11, color: colors.mutedForeground }}>
              Buscando...
            </Text>
          )}
          {exerciseSuggestions.length > 0 && (
            <View style={{ marginTop: 10, borderWidth: 1, borderColor: colors.border, borderRadius: 10, overflow: "hidden" as const }}>
              {exerciseSuggestions.slice(0, 10).map((it, idx) => (
                <Pressable
                  key={String(it.id)}
                  onPress={() => addExerciseFromCatalog(it)}
                  style={({ pressed }) => [
                    { paddingHorizontal: 12, paddingVertical: 10, backgroundColor: pressed ? `${colors.primary}11` : colors.secondary },
                    idx > 0 ? { borderTopWidth: 1, borderTopColor: colors.border } : null,
                  ]}
                >
                  <Text style={{ fontSize: 12, color: colors.foreground, fontWeight: "800" as const }}>
                    {it.name}
                  </Text>
                </Pressable>
              ))}
          ...
          <View style={{ height: 12 }} />
          <Text style={s.profileLabel}>EXERCÍCIOS DA FICHA</Text>
          {strengthExercises.length === 0 ? (
            <Text style={{ fontSize: 11, color: colors.mutedForeground, lineHeight: 16 }}>
              Sem exercícios ainda. Busque no catálogo acima e toque para adicionar.
            </Text>
          ) : (
            <View style={{ gap: 10 }}>
              {strengthExercises.map((ex, idx) => (
                <View key={`${idx}-${String(ex.catalogExerciseId ?? ex.name ?? "")}`} style={[s.strengthExerciseRow, { borderTopColor: colors.border }]}>
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
                    <Text style={{ flex: 1, fontSize: 12, color: colors.foreground, fontWeight: "800" as const }}>
                      {String(ex.name ?? "Exercício")}
                    </Text>
                    <Pressable
                      onPress={() => removeStrengthExercise(idx)}
                      style={({ pressed }) => [
                        s.removeBtn,
                        { opacity: pressed ? 0.7 : 1, borderColor: colors.border, backgroundColor: colors.secondary },
                      ]}
                    >
                      <Text style={[s.removeBtnText, { color: colors.mutedForeground }]}>REMOVER</Text>
                    </Pressable>
                  </View>
                  ...
                  <View style={s.miniRow}>
                    <TextInput
                      style={[s.miniField, { maxWidth: 70 }]}
                      value={ex.sets === null || ex.sets === undefined ? "" : String(ex.sets)}
                      onChangeText={(t) => updateStrengthExercise(idx, { sets: t ? Number(t) : null })}
                      placeholderTextColor={colors.mutedForeground}
                      placeholder="Séries"
                      keyboardType="number-pad"
                    />
                    <TextInput
                      style={s.miniField}
                      value={ex.reps ?? ""}
                      onChangeText={(t) => updateStrengthExercise(idx, { reps: t })}
                      placeholderTextColor={colors.mutedForeground}
                    ...
                    />
                    <TextInput
                      style={[s.miniField, { maxWidth: 90 }]}
                      value={ex.restSec === null || ex.restSec === undefined ? "" : String(ex.restSec)}
                      onChangeText={(t) => updateStrengthExercise(idx, { restSec: t ? Number(t) : null })}
                      placeholderTextColor={colors.mutedForeground}
                      placeholder="Desc (s)"
                      keyboardType="number-pad"
                    />
                  ...
                  <View style={s.miniRow}>
                    <TextInput
                      style={[s.miniField, { maxWidth: 90 }]}
                      value={ex.rpeTarget === null || ex.rpeTarget === undefined ? "" : String(ex.rpeTarget)}
                      onChangeText={(t) => updateStrengthExercise(idx, { rpeTarget: t ? Number(t) : null })}
                      placeholderTextColor={colors.mutedForeground}
                      placeholder="RPE"
                    ...
                    <TextInput
                      style={s.miniField}
                      value={ex.load ?? ""}
                      onChangeText={(t) => updateStrengthExercise(idx, { load: t })}
                      placeholderTextColor={colors.mutedForeground}
                      placeholder="Carga (ex.: 30kg)"
                      autoCapitalize="none"
                      autoCorrect={false}
                    />
                    <TextInput
                      style={[s.miniField, { maxWidth: 90 }]}
                      value={ex.tempo ?? ""}
                      onChangeText={(t) => updateStrengthExercise(idx, { tempo: t })}
                      placeholderTextColor={colors.mutedForeground}
                      placeholder="Tempo"
                      autoCapitalize="none"
                      autoCorrect={false}
                  ...
                  <View style={{ height: 8 }} />
                  <TextInput
                    style={[s.inputField, { fontSize: 12, minHeight: 44, textAlignVertical: "top" as any }]}
                    value={ex.notes ?? ""}
                    onChangeText={(t) => updateStrengthExercise(idx, { notes: t })}
                    placeholderTextColor={colors.mutedForeground}
                    placeholder="Observações (técnica, amplitude, dor, etc.)"
                    multiline
                  />
                </View>
              ))}
          ...
          <Pressable
            style={({ pressed }) => [
              s.importBtn,
              { opacity: pressed ? 0.7 : 1, backgroundColor: strengthSaving ? colors.border : colors.primary },
            ]}
          ...
          >
            ...
            <Text style={s.importBtnText}>SALVAR FICHA NO NEON</Text>
          </Pressable>
        </View>
