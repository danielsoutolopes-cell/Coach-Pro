        ...
        if (active.length === 0) {
          Alert.alert(
            ...
            [
              ...
              { text: "Ir para Tênis", onPress: () => router.push("/(tabs)/equipamentos" as any) },
            ]
          );
          return;
        }
  ...
  return (
    <View style={s.container}>
      <ShoePickerModal
        ...
        onPrimaryAction={() => {
          setShoePickOpen(false);
          router.push("/(tabs)/equipamentos" as any);
        }}
      />
      <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        <View style={s.header}>
          <View>
            <Text style={s.appTitle}>PROCOACH OS V6.1</Text>
              ...
              <View style={{ flex: 1 }}>
                ...
                {planToday ? (
                  <>
                    ...
                    {(() => {
                      const details = parseDetails(planToday.details_json);
                      ...
                      const hasMeta =
                        !!planToday.pace_target || !!planToday.rest_interval || !!planToday.treadmill_speed || !!bikeMin || !!volBody;
                      return (
                        <>
                          {hasMeta ? (
                            <Text style={{ fontSize: 11, color: colors.mutedForeground, marginTop: 4, lineHeight: 16 }}>
                              ...
                              {(planToday.pace_target || planToday.rest_interval) && planToday.treadmill_speed ? " · " : ""}
                              {planToday.treadmill_speed ? `Esteira: ${planToday.treadmill_speed}` : ""}
                              {(planToday.pace_target || planToday.rest_interval || planToday.treadmill_speed) && bikeMin ? " · " : ""}
                          ...
                          {!!blocks ? (
                            <View style={{ marginTop: 10, gap: 8 }}>
                              {[
                                ...
                                return (
                                  <View key={b.key} style={{ borderWidth: 1, borderColor: colors.border, borderRadius: 10, padding: 10 }}>
                                    <Text style={{ fontSize: 9, letterSpacing: 2, fontWeight: "800" as const, color: colors.mutedForeground }}>
                                      {b.title}
                                    </Text>
                                    <View style={{ marginTop: 6, gap: 4 }}>
                                      {list.slice(0, 8).map((seg: any, idx: number) => (
                                        <Text key={`${b.key}-${idx}`} style={{ fontSize: 12, color: colors.foreground, lineHeight: 16 }}>
                                          {String(seg?.label ?? "")}
                                        </Text>
                                      ))}
                                    </View>
