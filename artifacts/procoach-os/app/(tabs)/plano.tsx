...
import { ProCoachAPI, StrengthTemplate, StrengthTemplateCode } from "@/services/api";
import {
  DaySession,
  formatWeekRange,
...
function parseDetails(raw: unknown): any | null {
  if (!raw) return null;
...
function formatKm(v: unknown): string | null {
...
export default function PlanoScreen() {
  ...
  const [mode, setMode] = useState<"matriz" | "importado">("matriz");
  ...
  useEffect(() => {
    let cancelled = false;
    if (mode !== "importado") return;
    (async () => {
      setImportedLoading(true);
      setStrengthLoading(true);
      try {
        const from = getSaoPauloDayKey();
        const to = profile.targetRaceDate?.slice(0, 10) || undefined;
        const [r, st] = await Promise.all([
          ProCoachAPI.getPlan({ from, to }),
          ProCoachAPI.getStrengthTemplates().catch(() => null),
        ]);
        const list = Array.isArray(r.sessions) ? (r.sessions as any[]) : [];
        if (!cancelled) setImportedSessions(list.slice(0, 120));
        if (!cancelled) setStrengthTemplates(st?.templates ?? null);
      } catch {
        if (!cancelled) setImportedSessions([]);
        if (!cancelled) setStrengthTemplates(null);
      } finally {
  ...
  }, [mode, profile.targetRaceDate]);

  const detectStrengthTemplateCode = useCallback(
    (activity: string, structure: string | null | undefined): StrengthTemplateCode | null => {
      const text = `${activity ?? ""} ${structure ?? ""}`.toUpperCase();
      const m =
        text.match(/\bTREINO\s*([ABC])\b/) ||
        text.match(/\bMUSC(?:ULAÇÃO|ULACAO)?\s*([ABC])\b/) ||
        text.match(/\bFICHA\s*([ABC])\b/);
      const code = (m?.[1] ?? "").toUpperCase();
      if (code === "A" || code === "B" || code === "C") return code as StrengthTemplateCode;
      return null;
    },
    []
  );
  ...
  const s = StyleSheet.create({
    ...
    modeText: { fontSize: 10, fontWeight: "900" as const, letterSpacing: 2 },
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.modeRow}>
          <Pressable
            ...
            style={[
              s.modeBtn,
              mode === "matriz" ? { borderColor: colors.primary, backgroundColor: `${colors.primary}22` } : null,
            ]}
          >
            <Text style={[s.modeText, { color: mode === "matriz" ? colors.primary : colors.mutedForeground }]}>
              MATRIZ
            </Text>
          </Pressable>
          <Pressable
            onPress={() => setMode("importado")}
            style={[
              s.modeBtn,
              mode === "importado" ? { borderColor: colors.primary, backgroundColor: `${colors.primary}22` } : null,
            ]}
          >
            <Text style={[s.modeText, { color: mode === "importado" ? colors.primary : colors.mutedForeground }]}>
              IMPORTADO
            </Text>
          </Pressable>
        </View>
        ...
        {mode === "importado" ? (
          <View style={s.blockCard}>
            <View style={{ padding: 16 }}>
              <Text style={[s.blockPhase, { color: colors.primary }]}>PLANO IMPORTADO</Text>
              <Text style={s.blockMeta}>
                {importedLoading ? "Buscando do servidor..." : `${importedSessions.length} sessões`}
              </Text>
            </View>
            <View style={{ paddingHorizontal: 16, paddingBottom: 16 }}>
              {importedSessions.length === 0 ? (
                <Text style={s.phaseDesc}>
                  Nenhum treino encontrado no servidor. Importe na aba Check-in (Status).
                </Text>
              ) : (
                importedSessions.map((sess, idx) => (
                  <View key={`${sess.session_date}-${idx}`} style={{ paddingVertical: 10, borderTopWidth: idx === 0 ? 0 : 1, borderTopColor: colors.border }}>
                    <Text style={{ fontSize: 11, fontWeight: "800" as const, color: colors.foreground }}>
                      {sess.session_date}{sess.day_name ? ` · ${sess.day_name}` : ""}
                    </Text>
                    <Text style={{ fontSize: 12, color: colors.foreground, marginTop: 4 }}>
                      {sess.activity}{sess.planned_km ? ` · ${sess.planned_km}km` : ""}
                    </Text>
                    {(() => {
                      const details = parseDetails(sess.details_json);
                      const bikeMin = details?.bike?.minutes;
                      const bikeLabel = details?.bike?.label;
                      const blocks = details?.blocks ?? null;
                      const telem = details?.treadmillTelemetry ?? null;
                      const volBody =
                        telem?.rule === "A" ? formatKm(telem?.volumeBodyKm) : telem?.rule === "B" ? formatKm(telem?.volumeTotalKm) : null;
                      const volPanel = telem?.rule === "A" ? formatKm(telem?.volumePanelKm) : null;

                      const hasMeta =
                        !!sess.pace_target || !!sess.rest_interval || !!sess.treadmill_speed || !!bikeMin || !!volBody;
                      return (
                        <>
                          {hasMeta ? (
                            <Text style={{ fontSize: 11, color: colors.mutedForeground, marginTop: 4, lineHeight: 16 }}>
                              {sess.pace_target ? `Pace: ${sess.pace_target}` : ""}
                              {sess.pace_target && sess.rest_interval ? " · " : ""}
                              {sess.rest_interval ? `Rep: ${sess.rest_interval}` : ""}
                              {(sess.pace_target || sess.rest_interval) && sess.treadmill_speed ? " · " : ""}
                              {sess.treadmill_speed ? `Esteira: ${sess.treadmill_speed}` : ""}
                              {(sess.pace_target || sess.rest_interval || sess.treadmill_speed) && bikeMin ? " · " : ""}
                              {bikeMin ? `Bike: ${bikeMin}min${bikeLabel ? ` ${bikeLabel}` : ""}` : ""}
                              {(sess.pace_target || sess.rest_interval || sess.treadmill_speed || bikeMin) && volBody ? " · " : ""}
                              {volBody ? (volPanel ? `Corpo: ${volBody}km · Painel: ${volPanel}km` : `Volume: ${volBody}km`) : ""}
                            </Text>
                          ) : null}
                          {!!sess.structure ? (
                            <Text style={{ fontSize: 11, color: colors.mutedForeground, marginTop: 4, lineHeight: 16 }}>
                          ...
                          {!!blocks ? (
                            <View style={{ marginTop: 8, gap: 8 }}>
                              {[
                                { key: "warmup", title: "AQ" },
                                { key: "main", title: "PRINC" },
                                { key: "cooldown", title: "DQ" },
                              ].map((b) => {
                                const list = Array.isArray((blocks as any)[b.key]) ? (blocks as any)[b.key] : [];
                                if (!list.length) return null;
                                return (
                                  <View key={b.key} style={{ borderWidth: 1, borderColor: colors.border, borderRadius: 10, overflow: "hidden" as const }}>
                                    <View style={{ paddingHorizontal: 10, paddingVertical: 8, backgroundColor: colors.secondary }}>
                                      <Text style={{ fontSize: 10, fontWeight: "900" as const, letterSpacing: 2, color: colors.mutedForeground }}>
                                        {b.title}
                                      </Text>
                                    </View>
                                    <View style={{ paddingHorizontal: 10, paddingBottom: 10, paddingTop: 8, gap: 6 }}>
                                      {list.slice(0, 12).map((seg: any, i: number) => (
                                        <Text key={`${b.key}-${i}`} style={{ fontSize: 11, color: colors.mutedForeground, lineHeight: 16 }}>
                                          {String(seg?.label ?? "")}
                                        </Text>
                                      ))}
                                    </View>
                    ...
                    {(() => {
                      const code = detectStrengthTemplateCode(sess.activity, sess.structure);
                      if (!code) return null;
                      if (strengthLoading && !strengthTemplates) {
                        return (
                          <Text style={{ fontSize: 11, color: colors.mutedForeground, marginTop: 6 }}>
                            Carregando ficha {code}...
                          </Text>
                        );
                      }
                      const tpl = strengthTemplates?.[code] ?? null;
                      if (!tpl) {
                        return (
                          <Text style={{ fontSize: 11, color: colors.mutedForeground, marginTop: 6 }}>
                            Ficha {code} não encontrada. Cadastre em Check-in (Status) → Biblioteca de Força.
                          </Text>
                        );
                      }
                      if (!Array.isArray(tpl.exercises) || tpl.exercises.length === 0) {
                        return (
                          <Text style={{ fontSize: 11, color: colors.mutedForeground, marginTop: 6 }}>
                            Ficha {code} sem exercícios cadastrados.
                          </Text>
                        );
                      }
                      return (
                        <View style={{ marginTop: 8, borderWidth: 1, borderColor: colors.border, borderRadius: 10, overflow: "hidden" as const }}>
                          <View style={{ paddingHorizontal: 10, paddingVertical: 8, backgroundColor: colors.secondary }}>
                            <Text style={{ fontSize: 10, fontWeight: "900" as const, letterSpacing: 2, color: colors.mutedForeground }}>
                              FICHA {code}{tpl.name ? ` · ${tpl.name}` : ""}
                            </Text>
                          </View>
                          <View style={{ paddingHorizontal: 10, paddingBottom: 10, paddingTop: 8, gap: 6 }}>
                            {tpl.exercises.slice(0, 20).map((ex, i) => (
                              <Text key={`${code}-${i}`} style={{ fontSize: 11, color: colors.mutedForeground, lineHeight: 16 }}>
                                {i + 1}. {String(ex.name ?? "Exercício")}
                                {ex.sets ? ` · ${ex.sets}x` : ""}
                                {ex.reps ? ` ${ex.reps}` : ""}
