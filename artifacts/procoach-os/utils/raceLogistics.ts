...
export const RACE_ROLE: Record<RacePriority, { label: string; description: string; color: string }> = {
  P1: {
    ...
    description: "Objetivo máximo da temporada. Volume e intensidade convergem para esta prova, com polimento/tapering agressivo e foco total em performance.",
    color: "#FF5F00",
  },
  P2: {
    label: "CHECKPOINT / TESTE",
    description: "Check-point de luxo para calibrar zonas e ritmo da P1. Trate como simulado de alta qualidade sob fadiga controlada, sem exigir polimento de P1.",
    color: "#2196F3",
  },
  P3: {
    label: "TREINO DE LUXO",
    description: "Evento menor para ganhar casca de prova sem necessidade de descanso prévio. O objetivo é consistência e manutenção, com risco controlado.",
    color: "#9C27B0",
  },
};
...
export function validateRacePlacement(race: Race, p1DateISO: string): CycleValidation {
  ...
  if (race.priority === "P2") {
    if (week >= 15) {
      return { ok: false, warning: true, message: `⚠️ Semana ${week} (${phase}) — P2 muito próxima da P1. Faça como treino controlado (sem all-out) e priorize recuperação.` };
    }
    if (week >= 13) {
      return { ok: true, warning: false, message: `✅ Semana ${week} (${phase}) — ótimo checkpoint. Use para validar ritmo, géis e logística para a P1.` };
    }
    return { ok: true, warning: false, message: `✅ Semana ${week} (${phase}) — checkpoint sob fadiga controlada. O ciclo segue priorizando a P1.` };
  }

  if (race.priority === "P3") {
    if (week >= 15) {
      return { ok: false, warning: true, message: `⚠️ Semana ${week} (${phase}) — P3 muito próxima da P1. Avalie se vale o desgaste; se for, faça em ritmo seguro.` };
    }
    return { ok: true, warning: false, message: `✅ Semana ${week} (${phase}) — treino de luxo com risco controlado, sem interferir no foco da P1.` };
  }
